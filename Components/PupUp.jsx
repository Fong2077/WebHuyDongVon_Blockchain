...
//             <div className="flex items-center justify-end p-6 border-t border-soli border-slate-200 rounded-b">
//               <button
...
const PupUp = ({ setOpenModel, donateCampaign, donateFunction, getDonations }) => {
  const [amount, setAmount] = useState("");
  const [allDonationData, setAllDonationData] = useState([]);

  const createDonation = async () => {
    try {
      const data = await donateFunction(donateCampaign.pId, amount);
      console.log(data);
    } catch (error) {
      console.log(error);
    }
  ...
  return (
    <>
      <div className="justify-center items-center flex overflow-x-hidden overflow-y-auto fixed inset-0 z-50 outline-none focus:outline-none bgdonate">
        <div className="relative w-auto my-6 mx-auto max-w-3xl">
          <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
            <div className="flex items-start justify-between p-5 border-b border-solid border-slate-200 rounded-t">
            ...
            <div className="relative p-6 flex-auto">
              ...
              <input
                onChange={(e) => setAmount(e.target.value)}
                placeholder="Số tiền muốn donate (ETH)"
                required
                type="text"
                className="flex-grow w-full h-12 px-4 mb-2 transition duration-200 bg-white border border-gray-300 rounded shadow-sm appearance-none focus:border-deep-purple-accent-400 focus:outline-none focus:shadow-outline"
              />
              ...
              {allDonationData?.map((donate, i) => (
                <p key={i} className="my-4 text-slate-500 text-lg leading-relaxed">
                  {i + 1}. {donate.donation} ETH from {donate.donator.slice(0, 35)}...
                </p>
              ))}
            </div>
            ...
            <div className="flex items-center justify-end p-6 border-t border-solid border-slate-200 rounded-b">
              ...
              <button
                className="bg-blue-500 text-white font-bold uppercase text-sm px-6 py-3  shadow hover:shadow-lg outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150 rounded-3xl"
                type="button"
