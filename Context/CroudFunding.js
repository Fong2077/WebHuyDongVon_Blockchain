...
export const CrowdFundingProvider = ({ children }) => {
  ...
  // Kiểm tra nếu wallet đã kết nối
  ...
  const getUserDonations = async (account) => {
    try {
      const provider = new ethers.providers.JsonRpcProvider();
      const contract = fetchContract(provider);
      const campaigns = await contract.getCampaigns();
      ...
      for (let i = 0; i < campaigns.length; i++) {
        ...
        const numberOfDonations = donations[0].length;
        let donationCount = 0;
        let totalDonation = ethers.BigNumber.from(0);
  
        for (let j = 0; j < numberOfDonations; j++) {
          if (donations[0][j].toLowerCase() === account.toLowerCase()) {
            donationCount++;
            totalDonation = totalDonation.add(donations[1][j]);
          }
        }
        ...
        if (donationCount > 0) {
          userDonations.push({
            ...
            totalDonation: ethers.utils.formatEther(totalDonation.toString()),
          });
        }
      }
